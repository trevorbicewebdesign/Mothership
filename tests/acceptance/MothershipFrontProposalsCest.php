...
class MothershipFrontProposalsCest
{
    ...
    public function ViewAllProposalsPage(AcceptanceTester $I)
    {
        ...
        $I->seeNumberOfElements("main table#proposalsTable tbody tr", 1);

        // Confirm the table headers
        $I->see("#", "main table#proposalsTable thead tr th:nth-child(1)");
        $I->see("Name", "main table#proposalsTable thead tr th:nth-child(2)");
        $I->see("Client", "main table#proposalsTable thead tr th:nth-child(3)");
        $I->see("Account", "main table#proposalsTable thead tr th:nth-child(4)");
        $I->see("Type", "main table#proposalsTable thead tr th:nth-child(5)");
        $I->see("Status", "main table#proposalsTable thead tr th:nth-child(6)");
        $I->see("Created", "main table#proposalsTable thead tr th:nth-child(7)");

        // Confirm the table data
        $row = 1;
        $I->see("{$projectData['id']}", "main table#proposalsTable tbody tr:nth-child({$row}) td:nth-child(1)");
        $I->see("{$projectData['name']}", "main table#proposalsTable tbody tr:nth-child({$row}) td:nth-child(2)");
        $I->see("{$this->clientData['name']}", "main table#proposalsTable tbody tr:nth-child({$row}) td:nth-child(3)");
        $I->see("{$this->accountData['name']}", "main table#proposalsTable tbody tr:nth-child({$row}) td:nth-child(4)");
        $I->see("{$projectData['type']}", "main table#proposalsTable tbody tr:nth-child({$row}) td:nth-child(5)");
        $I->see("active", "main table#proposalsTable tbody tr:nth-child({$row}) td:nth-child(6)");
        $I->see("{$projectData['created']}", "main table#proposalsTable tbody tr:nth-child({$row}) td:nth-child(7)");
